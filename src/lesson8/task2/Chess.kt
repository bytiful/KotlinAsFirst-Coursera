...
data class Square(val column: Int, val row: Int) {
     ...
     */
    fun notation(): String {
        val l = listOf<String>("", "a", "b", "c", "d", "e", "f", "g", "h")
        return if (inside()) l[column] + "$row" else ""
    }
}
...
fun square(notation: String): Square {
    val col: Int
    val ro: Int
    val decod = mapOf('a' to 1, 'b' to 2, 'c' to 3, 'd' to 4, 'e' to 5, 'f' to 6, 'g' to 7, 'h' to 8)
    col = decod[notation[0]] ?: throw IllegalArgumentException("Некорректная нотация " + notation[0].toString())
    if (notation[1] > '0' || notation[1] < '9') ro = notation[1].toString().toInt() else throw IllegalArgumentException("Некорректная нотация")
    val result = Square(col, ro)
    return result
}
...
fun rookMoveNumber(start: Square, end: Square): Int {
    if (!start.inside()) throw IllegalArgumentException("Некорректный аргумент ${start.column}${start.row}")
    if (!end.inside()) throw IllegalArgumentException("Некорректный аргумент ${end.column}${end.row}")
    var result = 0
...
fun rookTrajectory(start: Square, end: Square): List<Square> {
    ...
    if (start.row != end.row) result.add(Square(col, end.row))
    return result
}
...
fun bishopMoveNumber(start: Square, end: Square): Int {
    ...
    val dx = max(start.column, end.column) - min(start.column, end.column)
    val dy = max(start.row, end.row) - min(start.row, end.row)
    when {
        (dx + dy) % 2 > 0 -> return -1
        (dx == 0) && (dy == 0) -> return 0
        dx == dy -> return 1
...
fun bishopTrajectory(start: Square, end: Square): List<Square> {
    ...
    when (bishopMoveNumber(start, end)) {
        ...
        2 -> {
            ...
            val dy = max(start.row, end.row) - min(start.row, end.row)
            when {
                dx > dy -> { //y меняет направление
                    when {
                        start.column > end.column -> { //справа налево
                            val rc = start.column - (dx / 2)
                            var rr = start.row - (dx / 2)
                            if (!Square(rc, rr).inside()) {
                                rr = start.row + (dx / 2)
                            }
                            result.add(Square(rc, rr))
                        }
                        else -> { //слева направо
                            val rc = start.column + (dx / 2)
                            var rr = start.row - (dx / 2)
                ...
                else -> { //x меняет направление
                    when {
                        start.row > end.row -> { //сверху вниз
                            var rc = start.column - (dy / 2)
                            val rr = start.row - (dy / 2)
                            if (!Square(rc, rr).inside()) {
                                rc = start.column + (dy / 2)
                            }
                            result.add(Square(rc, rr))
                        }
                        else -> { //снизу вверх
...
fun kingMoveNumber(start: Square, end: Square): Int {
    if (start.inside() && end.inside()) {
        ...
        val dy = max(start.row, end.row) - min(start.row, end.row)
        return max(dx, dy)
    } else throw IllegalArgumentException("Неправильные аргументы")
}
