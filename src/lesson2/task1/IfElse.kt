...
fun segmentLength(a: Int, b: Int, c: Int, d: Int): Int {
    return when {
        c > b || a > d -> -1 //начало одного отрезка за пределами конца другого
        c in a..b && d >= b -> b - c //пересекаются или совпадают, первый раньше начинается или кончается
        a >= c && b <= d -> b - a //первый отрезок внутри второго
        c >= a && d <= b -> d - c //второй отрезок внутри первого
